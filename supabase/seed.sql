...
-- Latest measured/platform-calculated performance numbers, shown on
-- /journal's Performance Benchmarks section. Meant to be added to (a new
-- recorded_on with new rows) via a future one-off SQL file, not edited in
-- place here — see performance_snapshots' comment in schema.sql.
...
) as v (recorded_on, category, metric_key, label, value_display, value_numeric, unit, is_measured, display_order)
where not exists (select 1 from public.performance_snapshots);
